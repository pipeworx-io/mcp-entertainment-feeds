 ...
 *
 * Entertainment & culture — film, TV, music and books.
...
const PACK_LABEL = 'Entertainment Feeds';
...
const FEEDS: Record<string, FeedDef> = {
  ...
  consequence: { title: "Consequence", url: "https://consequence.net/feed/", category: "music", source: "Consequence", description: "Music & pop culture." },
  'npr-music': { title: "NPR Music", url: "https://feeds.npr.org/1039/rss.xml", category: "music", source: "NPR Music", description: "Music features & reviews from NPR." },
  'book-riot': { title: "Book Riot", url: "https://bookriot.com/feed/", category: "books", source: "Book Riot", description: "Book recommendations & news." },
};

const tools: McpToolExport['tools'] = [
  {
    name: 'list_feeds',
    description: 'List the curated entertainment & culture feeds (id, title, category, source). Optionally filter by category (books, entertainment, music) or keyword. Pass an id to read_feed.',
    inputSchema: {
      type: 'object',
      properties: {
        category: { type: 'string', description: 'Filter by category: books, entertainment, music.' },
  ...
  {
    name: 'read_feed',
    description: 'Read a curated entertainment & culture feed by its id (from list_feeds). Returns normalized items (title, link, published, summary). Optionally filter items by keyword.',
    inputSchema: {
      type: 'object',
      properties: {
